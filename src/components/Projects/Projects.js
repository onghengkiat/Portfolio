...
export default function Projects({ setActive }) {
    ...
    const [numberOfOtherProjectShown, setNumberOfOtherProjectShown] = useState(INITIAL_NUMBER_OF_OTHER_PROJECT_TO_SHOW);

    useEffect( () => {
        setActive(2);
        setOtherProjectsDataShown(otherProjectsData.slice(0, numberOfOtherProjectShown));
    ...
    const viewMoreOrViewLessOtherProject = () => {
        ...
        const returnToInitialState = (numberOfOtherProjectShown - NUMBER_OF_LOAD_MORE_PROJECTS) < INITIAL_NUMBER_OF_OTHER_PROJECT_TO_SHOW

        if (numberOfOtherProjectShown < NUMBER_OF_OTHER_PROJECT) { 
            // There are more projects to be shown, view more button is clicked
            setNumberOfOtherProjectShown(numberOfOtherProjectShown + NUMBER_OF_LOAD_MORE_PROJECTS);
    ...
    return(
        <div className="pageWrapper" id="projectsPageWrapper">
                ...
                {otherProjectsDataShown.map((projectCard, index) => 
                    <li className="projectCard">
                        <h3 className="projectCardName">
                            {projectCard.name}
                            <a href={projectCard.githubUrl} title="Read the codes" target="_blank"><AiFillGithub/></a>
                            {
                                projectCard.websiteUrl ? 
                                <a href={projectCard.websiteUrl} title="Visit the website" target="_blank"><AiOutlineLink/></a> :
                                null
                            }
                        </h3>
            ...
            <div className="otherProjectButtonBlock">
                <button className="viewMoreProjectButton" onClick={viewMoreOrViewLessOtherProject}>
                    {numberOfOtherProjectShown >= NUMBER_OF_OTHER_PROJECT ? "View Less" : "View More"}
                </button>
            </div>
        </div>
    );
